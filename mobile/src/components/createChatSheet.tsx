...
import { Button, H3, Input, Label, ScrollView, Sheet, Spinner, TextArea, XGroup, YStack } from "tamagui";
import { z } from "zod";
...
export function CreateChatSheet() {
    ...
    const { watch, setValue, handleSubmit, reset, formState: { errors } } = useForm<formValues>({ resolver: zodResolver(schema) });
    const { add, load, isCreating } = useChatStore();
    ...
    return (
        <>
            <Button size="$5" onPress={handleOpen} icon={<Plus size="$2" />}>Start a new Chat</Button>
            <Sheet modal forceRemoveScrollEnabled={open} open={open} onOpenChange={handleOpen} zIndex={100_000} animation="medium" dismissOnSnapToBottom>
                ...
                <Sheet.Frame p="$5">
                    <H3 textAlign="center">Create Chat</H3>
                    ...
                    <YStack mt="$5">
                        <Button disabled={isCreating} onPress={handleSubmit(handleCreate)} size="$5" backgroundColor="$color4">
